...
def conda2pip(ymlFile="oephys2nwb.yml", return_lists=False):
    """
    Convert conda environment YAML file to pip-style requirements.txt
    ...
    if return_lists:
        ...
        # Lists tend to be used in setup commands; adapt git repo dependencies
        # to be compatible with `install_requires`
        required = [dep.split('/')[-1] + " @ " + dep if 'git+' in dep else dep for dep in required]
        testing = [dep.split('/')[-1] + " @ " + dep if 'git+' in dep else dep for dep in testing]
